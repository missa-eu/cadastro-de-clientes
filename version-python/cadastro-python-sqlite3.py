...
class ClienteApp(tk.Tk):
    ...
    def carregar_cliente_selecionado(self, event):
        ...
        self.cursor.execute('SELECT nome, endereco, rede_social, email, data_cadastro FROM clientes WHERE id=?', (cliente_id,))
    ...
    def salvar_cliente(self):
        ...
        if nome and email and data_cadastro:
            ...
            if self.clientes_tree.selection():
                ...
                self.cursor.execute('''
                    UPDATE clientes SET nome=?, endereco=?, rede_social=?, email=?, data_cadastro=?
                    WHERE id=?
                ''', (*cliente, cliente_id))
            else:
                self.cursor.execute('''
